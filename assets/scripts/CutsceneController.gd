...
func on_tween2_finished():
	...
	tween2.tween_property($"CanvasModulate", "color", Color.BLACK, 2)
	tween2.tween_property($"CanvasModulate", "color", Color.RED, 2)
	tween2.tween_property($"CanvasModulate", "color", Color.BLACK, 2)
	tween2.parallel()
	tween2.tween_property($CharacterBody2D, "position", Vector2(0, -360), 2).set_ease(Tween.EASE_IN)
